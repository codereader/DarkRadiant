...
NOTE: It is possible create _three_ split points before executing the split,
...
DarkRadiant allows each of the 12 edges of a rectangular brush to be moved,
allowing you to quickly create shapes like triangles or trapeziums. This is
...
There are two tabs on the editing panel which show information about textures,
and both allow you to apply a given texture to the selected brush(es).

*Media*:: A browseable tree view which shows all of the textures available
within the game installation, organised in the hierarchy specified within the
game files. Selecting a texture in this tree will show a small preview swatch in
the widget at the bottom of the panel, along with some metadata about the
particular texture definition.
+ 
To apply a texture from the *Media* tab, either *Double-click* on a texture name
in the tree, or *Right-click* and choose *Apply to selection*.

*Textures*:: A scrollable canvas which shows square or rectangular preview
swatches of all the textures which are currently loaded by DarkRadiant. When
DarkRadiant first starts up, no textures are loaded and this panel is empty. By
right-clicking on a texture or texture folder in the *Media* tab, you can access
a context menu which allows you to load textures into the *Textures* tab.
+
To apply a texture from this tab, simply *Left-click* on the desired texture
while one or more brushes is selected..
