...
void TransformDialog::populateWindow() {
	...
	GtkTable* rotateTable = GTK_TABLE(gtk_table_new(3, 2, false));
    gtk_table_set_col_spacings(rotateTable, 12);
    gtk_table_set_row_spacings(rotateTable, 6);
    
    // Pack it into an alignment so that it is indented
	GtkWidget* rotAlignment = gtkutil::LeftAlignment(GTK_WIDGET(rotateTable), 18, 1.0); 
	gtk_box_pack_start(GTK_BOX(_dialogVBox), GTK_WIDGET(rotAlignment), false, false, 0);
    
    _entries["rotateX"] = createEntryRow(LABEL_ROTX, rotateTable, 0);
    _entries["rotateY"] = createEntryRow(LABEL_ROTY, rotateTable, 1);
    _entries["rotateZ"] = createEntryRow(LABEL_ROTZ, rotateTable, 2);
    ...
    // Setup the table with default spacings
	GtkTable* scaleTable = GTK_TABLE(gtk_table_new(3, 2, false));
    gtk_table_set_col_spacings(scaleTable, 12);
    gtk_table_set_row_spacings(scaleTable, 6);
    
    // Pack it into an alignment so that it is indented
	GtkWidget* scaleAlignment = gtkutil::LeftAlignment(GTK_WIDGET(scaleTable), 18, 1.0); 
	gtk_box_pack_start(GTK_BOX(_dialogVBox), GTK_WIDGET(scaleAlignment), false, false, 0);
    
    _entries["scaleX"] = createEntryRow(LABEL_SCALEX, scaleTable, 0);
    _entries["scaleY"] = createEntryRow(LABEL_SCALEY, scaleTable, 1);
    _entries["scaleZ"] = createEntryRow(LABEL_SCALEZ, scaleTable, 2);
	...
	const std::string& label, GtkTable* table, int row) 
{
	...
	// Create the control buttons (zero spacing hbox)
	{
		...
		entryRow.smaller = ControlButtonPtr(new gtkutil::ControlButton("arrow_left.png"));
		gtk_widget_set_size_request(*entryRow.smaller, 15, 24);
		gtk_box_pack_start(GTK_BOX(hbox), *entryRow.smaller, false, false, 0);
		
		entryRow.larger = ControlButtonPtr(new gtkutil::ControlButton("arrow_right.png"));
...
void TransformDialog::update() {
	...
	bool sensitive = (_selectionInfo.entityCount == 0 && _selectionInfo.totalCount > 0);

	gtk_widget_set_sensitive(GTK_WIDGET(_dialogVBox), sensitive);
}

void TransformDialog::selectionChanged(scene::Instance& instance) {
	update();
}
...
gboolean TransformDialog::onDelete(GtkWidget* widget, GdkEvent* event, TransformDialog* self) {
