	...
	_previewHeight(0)
{
	...
	_glWidget->signal_expose_event().connect(sigc::mem_fun(*this, &AnimationPreview::callbackGLDraw));
	_glWidget->signal_motion_notify_event().connect(sigc::mem_fun(*this, &AnimationPreview::callbackGLMotion));
	_glWidget->signal_scroll_event().connect(sigc::mem_fun(*this, &AnimationPreview::callbackGLScroll));
	_glWidget->signal_size_allocate().connect(sigc::mem_fun(*this, &AnimationPreview::onSizeAllocate));
	...
	_startButton->signal_clicked().connect(sigc::mem_fun(*this, &AnimationPreview::callbackStart));
	...
	_pauseButton->signal_clicked().connect(sigc::mem_fun(*this, &AnimationPreview::callbackPause));
	...
	_stopButton->signal_clicked().connect(sigc::mem_fun(*this, &AnimationPreview::callbackStop));
	...
	nextButton->signal_clicked().connect(sigc::mem_fun(*this, &AnimationPreview::callbackStepForward));
	...
	prevButton->signal_clicked().connect(sigc::mem_fun(*this, &AnimationPreview::callbackStepBack));
	...
	//_showAxesButton->signal_toggled().connect(sigc::mem_fun(*this, &AnimationPreview::callbackToggleAxes));
...
void AnimationPreview::setSize(int width, int height)
{
...
void AnimationPreview::initialisePreview()
{
...
void AnimationPreview::startPlayback()
{
	if (_timer.isEnabled())
	{
...
void AnimationPreview::stopPlayback()
{
	_renderSystem->setTime(0);
	_timer.disable();
...
void AnimationPreview::callbackStart()
{
	startPlayback();
}

void AnimationPreview::callbackPause()
{
	// Disable the button
	_pauseButton->set_sensitive(false);
...
void AnimationPreview::callbackStop()
{
	stopPlayback();
}

void AnimationPreview::callbackStepForward()
{
	// Disable the button
	_pauseButton->set_sensitive(false);
...
void AnimationPreview::callbackStepBack()
{
	// Disable the button
	_pauseButton->set_sensitive(false);
...
void AnimationPreview::onSizeAllocate(Gtk::Allocation& allocation)
{
	_previewWidth = allocation.get_width();
	_previewHeight = allocation.get_height();
}

Matrix4 AnimationPreview::getProjectionMatrix(float near_z, float far_z, float fieldOfView, int width, int height)
{
...
bool AnimationPreview::callbackGLDraw(GdkEventExpose* ev)
{
	...
	// TODO _particle->update(_previewTimeMsec, *_renderSystem, _rotation);

	// Front-end render phase, collect OpenGLRenderable objects from the "scene"
	_model->renderSolid(_renderer, _volumeTest);
...
gboolean AnimationPreview::_onFrame(gpointer data)
{
	AnimationPreview* self = reinterpret_cast<AnimationPreview*>(data);
...
void AnimationPreview::drawTime()
{
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
...
bool AnimationPreview::callbackGLMotion(GdkEventMotion* ev)
{
...
bool AnimationPreview::callbackGLScroll(GdkEventScroll* ev)
{
	if (_model == NULL) return false;

	// Scroll increment is a fraction of the AABB radius
	float inc = 1.0f; //static_cast<float>(_particle->getBounds().getRadius()) * 0.1f;
