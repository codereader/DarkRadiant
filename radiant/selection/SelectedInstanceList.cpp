...
scene::Instance& SelectedInstanceList::ultimate() {
	if (size() == 0) {
...
scene::Instance& SelectedInstanceList::ultimate() const {
	if (size() == 0) {
		return *(end()->first); // return invalid iterator
	}
...
scene::Instance& SelectedInstanceList::penultimate() const {
	if (size() <= 1) {
		return *(end()->first); // return invalid iterator
	}
...
scene::Instance& SelectedInstanceList::penultimate() {
	if (size() <= 1) {
		return *(end()->first); // return invalid iterator
	}
...
void SelectedInstanceList::append(scene::Instance& selected) {
	time++;
	MapType::insert(value_type(&selected, time));
}

void SelectedInstanceList::erase(scene::Instance& selected) {
	iterator last;
...
unsigned long SelectedInstanceList::time = 1;
