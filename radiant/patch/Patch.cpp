...
void Patch::TranslateTexture(float s, float t)
{
  undoSave();
    ...
    s = -1 * s / m_state->getIShader()->getEditorImage()->width;
    t = t / m_state->getIShader()->getEditorImage()->height;
...
void Patch::NaturalTexture() {
	 ...
	 */	
	{
		float fSize = (float)m_state->getIShader()->getEditorImage()->width * defaultScale;
	...
	{
		float fSize = -(float)m_state->getIShader()->getEditorImage()->height * defaultScale;
...
void Patch::ProjectTexture(int nAxis) {
	...
	float fWidth = 1 / (m_state->getIShader()->getEditorImage()->width * defaultScale);
	float fHeight = 1 / (m_state->getIShader()->getEditorImage()->height * -defaultScale);
