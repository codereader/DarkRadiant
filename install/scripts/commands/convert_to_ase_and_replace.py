...
__commandName__ = 'aseConvertAndReplace'
__commandDisplayName__ = 'Convert to ASE...'
...
def execute():
    ...
    suggestedName = found_func_static.getEntity().getKeyValue("name")
    dialog.setElementValue(fileHandle, suggestedName + ".ase")
    ...
    if dialog.run() == Dialog.OK:
        ...
        if overwrite:
            ...
            # center objects at found func_static origin
...
{5}'''.format(script, version, scene, len(shaderlist), materials, geomobjects)
            ...
            GlobalCommandSystem.execute('DeleteSelection')

            # Create a new func_static
            func_static = GlobalEntityCreator.createEntity("func_static")
            ...
            relativePath = relativePath.replace(GlobalGameManager.getModPath(), '')
            relativePath = relativePath.replace(GlobalGameManager.getModBasePath(), '')
            print('Relative path is ' + relativePath)

            func_static.getEntity().setKeyValue("model", relativePath)

            # Insert the entity as child to the root node
            func_static.addToContainer(GlobalSceneGraph.root())
